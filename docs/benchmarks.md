...
How to run, what the numbers mean, and the current state of the spec
section 7 targets. Numbers are medians of `-count=3` runs. Loopback benchmarks are
...
|---|---|---|---|
| ProxyRoundTrip-20 | 112,327 ns/op | 112.0 / 112.3 / 139.0 us | ~8,900 round trips/s |
| ProxyPipelined-20 | 55,942 ns/op | 55.9 / 67.0 / 51.8 us | ~17,900 packets/s end-to-end |

Cross-run spread: a second `-count=3` invocation minutes later gave medians
of 129.5 us (RoundTrip) and 43.9 us (Pipelined, ~22,800 packets/s), a +/-20%
...
flight cuts per-packet cost roughly 2-3x versus a synchronous round trip.
...
locally:
    ...
    # or: gh workflow run ci && gh run watch
...
## Spec section 7 targets vs. current state
...
in `/proc/net/snmp` or `netstat -s` while the process is healthy) or (b) a
