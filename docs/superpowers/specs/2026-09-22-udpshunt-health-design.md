...
bdhg 生产事故（2026-09-22，room-temp 监听器，~1200 会话）：三个后端在 7 秒内连环
标记 down，全部会话被强制关闭。最初归因"探测超时误判"**不成立**：

- 事故时三个后端刚完成 `rise: 2` 恢复（08:06:01-04 三个 backend_up），说明探测正在成功。
...
3. **down ⇒ 强杀会话**：误判代价被放大（3000 会话瞬间蒸发）。

另有一处归因错误：`listener.go:283-292` 下行转发给**客户端**失败（移动 NAT 失联，
常态）也被计入后端错误 —— 客户端侧问题不该由后端背锅。
